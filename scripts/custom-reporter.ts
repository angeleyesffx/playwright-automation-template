...
export default class EnhancedReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    ...
    if (projectName === "project" || projectName === "unknown") {
      ...
      if (this.config?.projects) {
        const matchingProject = this.config.projects.find(
          (p) => p.use?.baseURL === baseUrl,
        );
        if (matchingProject) {
    ...
    const url =
      test.parent?.project()?.use?.baseURL ||
      (testType === "api" ? "API Tests" : "unknown");
    ...
    this.testResults.push({
      ...
      status:
        (result.status as "passed" | "failed" | "skipped" | "unknown") ||
        "unknown",
  ...
  async onExit(): Promise<void> {
    ...
    const browserTests = this.testResults.filter(
      (t) => t.testType === "browser",
    );
    ...
    const summaryData = {
      ...summary,
      environment:
        this.runningEnvironment !== "Unknown"
