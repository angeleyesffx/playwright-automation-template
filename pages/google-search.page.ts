...
export class GoogleSearchPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.searchInput = page
      .locator('#searchInput, input[name="search"]')
      .first();
    this.resultsContainer = page
      .locator(".mw-search-results, #mw-content-text")
      .first();
    this.resultHeadings = page.locator(
      ".mw-search-result-heading a, #firstHeading",
    );
  }
  ...
  async search(query: string): Promise<void> {
    await super.navigate(
      `/wiki/Special:Search?search=${encodeURIComponent(query)}&ns0=1`,
    );
