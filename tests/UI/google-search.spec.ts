...
test.describe("Wikipedia Search @smoke @ui", () => {
  test("should display search input on the main page", async ({
    googleSearchPage,
  }) => {
  ...
  test("should display results after searching", async ({
    googleSearchPage,
  }) => {
  ...
  test("should include the query in the URL after search", async ({
    googleSearchPage,
  }) => {
  ...
  test("should return a non-empty title for the first result", async ({
    googleSearchPage,
  }) => {
...
test.describe("Wikipedia Search — equivalence partitioning @ui", () => {
  for (const query of VALID_QUERIES) {
    test(`should return results for "${query}"`, async ({
      googleSearchPage,
    }) => {
...
test.describe("Wikipedia Search — negative cases @ui", () => {
  test("should URL-encode special characters in the search query", async ({
    googleSearchPage,
  }) => {
  ...
  test("should display the results page for an unrecognised query", async ({
    googleSearchPage,
  }) => {
  ...
  test("should update the URL when performing a second consecutive search", async ({
    googleSearchPage,
  }) => {
    ...
    expect(googleSearchPage.currentUrl()).toMatch(
      /search=JavaScript|\/wiki\/JavaScript/i,
    );

    await googleSearchPage.search("TypeScript");
    expect(googleSearchPage.currentUrl()).toMatch(
      /search=TypeScript|\/wiki\/TypeScript/i,
    );
  });
});
