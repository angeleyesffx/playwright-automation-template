export class ResponseExtractor {
  ...
  static extractValues(
    ...
    return Object.fromEntries(
      Object.entries(paths).map(([name, path]) => [
        name,
        this.extractValue(response, path),
      ]),
    );
  }
  ...
  static extractId(response: Record<string, unknown>): number | string | null {
    for (const field of ["id", "Id", "ID", "resourceId", "entityId"]) {
      if (field in response && response[field] != null)
        return response[field] as number | string;
    }
    return null;
  }
  ...
  static validateRequired(
    ...
    return requiredFields.filter(
      (field) => this.extractValue(response, field) == null,
    );
  }
}
...
export class DataChainBuilder {
  ...
  extractAndStore(
    ...
    const value = path
      ? ResponseExtractor.extractValue(response, path)
      : response;
    this.set(key, value);
    return value;
  }
